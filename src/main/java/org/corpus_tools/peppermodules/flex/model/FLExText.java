 ...
 * ## Salt mapping
 * 
 * 'item' elements, i.e., linguistically relevant annotations, are
 * mapped onto Salt {@link SAnnotation}s. These have a `namespace`,
 * a `name` and a `value`.
 * 
 * During the mapping, an 'item's 'lang' is mapped onto
 * the {@link SAnnotation} `namespace`, and its 'type'
 * is mapped onto the {@link SAnnotation} `name`.
 * 
 * The *level* of the annotation in the FLEx XML structure
 * is represented by assigning the annotation's container
 * (i.e., the respective node) an **item layer**. This
 * can be used to retrieve the level of the annotation
 * during downstream manipulation and export.
 * 
 * ### Item layers
 * 
 * Item layers represent the levels of the FLEx XML
 * which can have 'item' elements, i.e., linguistically
 ...
 * - `interlinear-text`
 * 
 * 'item' elements on the `interlinear-text` level
 * cannot be mapped to annotations whose container
 * (an {@link SDocument}) cannot be added to its
 * internal layer. Therefore, 'item's on the
 * `interlinear-text` level are annotations on
 * the {@link SDocument}. This information can be used
 * to retrieve the level of the annotation during
 * downstream manipulation and export.
 * 
 * ### Token layers
 * 
 * As the import of FLEx XML creates two token
 * layers in Salt (and also two different data 
 * source nodes) - namely for lexical and for
 * morphological tokens - these are being added
 * to a dedicated layer respectively. These are
...
public interface FLExText {
	 ...
	 * 
	 * This corresponds to the Salt element `SCorpus`. As all FLExText files
	 * start with this, the corpus-document containment relationships depend on
	 * the number of FLExText *Text*s exported at any one time.
	 * 
	 * Additionally, the {@link FLExImporter} also works on directories, in case
	 * of which all files contained in a directory become a sub-corpus.
	 */
